...
async def generate_first_frame_prompt(
    ...
    system_prompt = (
        ...
        "frame of the video. An IP-Adapter also injects the product's visual appearance.\n\n"
        + visual_instruction + "\n"
        "CRITICAL — PRODUCT TYPE FRAMING:\n"
        "If the product is a plush toy, keychain, figurine, or small physical item:\n"
        "  - ALWAYS use words like 'plush toy', 'soft toy keychain', 'small toy'\n"
        "  - NEVER describe it as a 'creature', 'character', or living being\n"
        "  - This prevents Flux from generating a horror/monster version of the product\n"
        "  - Example: 'fuzzy green plush toy keychain' NOT 'green plankton creature'\n\n"
        "RULES:\n"
        ...
        "3. Always frame the product as a manufactured TOY/PRODUCT, not a living thing\n"
        ...
        "  → 'Tiny fuzzy green plankton plush toy keychain clipped to dark zipper, "
        "one large yellow eye, close-up, warm bedroom light, soft bokeh'\n\n"
        "BAD examples:\n"
        "  → 'A green plankton creature with one eye' ← sounds like living monster!\n"
        "  → 'A keychain on a bag' ← too vague\n"
        "  → 'A hand clips the keychain' ← motion word"
    )
...
async def score_prompt_fidelity(prompt: str) -> float:
    ...
    """
    # IP-Adapter scale range for product advertising:
    #   < 0.55 → too weak: product style barely visible (generic scene)
    #   > 0.70 → too strong: CLIP injects creature/character features too aggressively
    #             → uncanny-valley / horror-looking output for plush/character products
    #   0.55–0.70 sweet spot: product colour and shape visible, scene looks natural
    #
    # Note: a negative_prompt is applied in Flux to further prevent horror/monster output
    # regardless of ip_weight — this range focuses on the scale balance only.
    WEIGHT_MIN     = 0.55
    WEIGHT_MAX     = 0.70
    WEIGHT_DEFAULT = 0.62  # Returned on any error (balanced)
