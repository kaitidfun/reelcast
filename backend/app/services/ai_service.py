...
async def generate_first_frame_prompt(
    ...
    system_prompt = (
        ...
        "2. Describe the product using the product name and description provided\n"
        "   (colour, shape, character details — be specific)\n"
    ...
    try:
        ...
        result = await loop.run_in_executor(None, _generate)
        result = result[:220]
        logger.info(f"[FirstFramePrompt] Generated ({len(result)} chars): {result[:80]}...")
        return result
