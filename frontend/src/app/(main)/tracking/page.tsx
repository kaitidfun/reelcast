...
export default function Tracking() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-baseline justify-between gap-2"><div><h2 className="font-display text-lg font-semibold text-foreground">Performance overview</h2><p className="text-sm text-muted-foreground">Views and engagement come from social platforms; outbound clicks come from ReelCast redirects.</p></div><p className="text-sm text-muted-foreground">{loading ? "–" : number.format(totals?.published_distributions ?? 0)} Published Distributions</p></div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 xl:grid-cols-6">
          ...
          <StatCard compact icon={MousePointerClick} label="Outbound Click" value={loading ? "-" : number.format(totals?.clicks ?? 0)} delay={0.15} />
      ...
      <section className="grid gap-5 xl:grid-cols-5">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-5 shadow-card sm:p-6 xl:col-span-3">
          <div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-info/10"><BarChart3 className="h-4 w-4 text-info" /></div><div><h2 className="font-display text-lg font-semibold text-foreground">Performance over time</h2><p className="text-xs text-muted-foreground">Views, outbound clicks, and confirmed affiliate orders across the selected period.</p></div></div>
          {chartData.length === 0 ? <EmptyState message="Metrics appear after a social sync or a ReelCast redirect request." /> : <div className="mt-5 h-64 sm:h-72"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}><defs><linearGradient id="trackingViews" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="hsl(var(--primary))" stopOpacity={0.3} /><stop offset="100%" stopColor="hsl(var(--primary))" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" strokeOpacity={0.5} /><XAxis dataKey="date" tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} /><YAxis tick={{ fontSize: 12, fill: "hsl(var(--muted-foreground))" }} axisLine={false} tickLine={false} tickFormatter={(value) => number.format(value)} /><Tooltip formatter={(value: number) => [value.toLocaleString(), undefined]} /><Area type="monotone" dataKey="views" name="Views" stroke="hsl(var(--primary))" strokeWidth={2.5} fill="url(#trackingViews)" /><Area type="monotone" dataKey="clicks" name="Outbound Click" stroke="hsl(var(--info))" strokeWidth={2} fill="transparent" /><Area type="monotone" dataKey="orders" name="Affiliate Orders" stroke="hsl(var(--success))" strokeWidth={2} fill="transparent" /></AreaChart></ResponsiveContainer></div>}
        </motion.div>
